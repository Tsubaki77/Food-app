...
function displayMeal(meal, type) {
    ...
    mealList.addEventListener('click', (e) => {
        if (e.target.id === meal.idMeal) {
            // Si le type est l'ajout on ajoute le plat aux favoris et 
            // on ajoute le tableau au local storage 
            if (type === "Add to") {
                favorites.push(meal.idMeal)
                localStorage.setItem('favorites', JSON.stringify(favorites)) 

            // Si le type est la suppression (on est dans les favoris du coup)
            // alors on supprime le plat du storage et on réaffiche les favs
